...
from utils.metrics_dcd import calc_dcd
        ...
        for idx, (taxonomy_ids, model_ids, data) in enumerate(train_dataloader):
            ...
            if dataset_name == 'PCN' or dataset_name == 'Completion3D' or dataset_name == 'Projected_ShapeNet':
                ...
                partial, _ = misc.seprate_point_cloud(gt, npoints, [int(npoints * 1 / 4), int(npoints * 3 / 4)],
                                                      fixed_points=None)   # misc.seprate_point_cloud_train
...
def test(base_model, test_dataloader, ChamferDisL1, ChamferDisL2, args, config, logger=None):
    ...
    test_losses = AverageMeter(['SparseLossL1', 'SparseLossL2', 'DenseLossL1', 'DenseLossL2'])
    test_metrics = AverageMeter(Metrics.names())
    test_metrics_dcd = AverageMeter(['DCD'])


    category_metrics = dict()
    category_metrics_dcd = dict()
    n_samples = len(test_dataloader)  # bs is 1
    ...
    with torch.no_grad():
        for idx, (taxonomy_ids, model_ids, data) in enumerate(test_dataloader):
            ...
            if dataset_name == 'PCN':
                ...
                _metrics = Metrics.get(dense_points, gt, require_emd=False)
############################################################################################################
                dcd, _, _ = calc_dcd(dense_points, gt)
                dcd = dcd.mean().item()
                _metrics_dcd = [dcd]
############################################################################################################
                ...
                if taxonomy_id not in category_metrics:
                    ...
                    category_metrics_dcd[taxonomy_id] = AverageMeter(['DCD'])
                category_metrics[taxonomy_id].update(_metrics)
                category_metrics_dcd[taxonomy_id].update(_metrics_dcd)
...
############################################################################################################
            ...
            if dataset_name != 'KITTI' and (idx + 1) % 200 == 0:
                print_log('Test[%d/%d] Taxonomy = %s Sample = %s Losses = %s Metrics = %s' %
                          (idx + 1, n_samples, taxonomy_id, model_id, ['%.4f' % l for l in test_losses.val()],
                           ['%.4f' % m for m in _metrics]), logger=logger)
                print_log('Test_dcd[%d/%d] Taxonomy = %s Sample = %s Losses = %s Metrics = %s' %
                          (idx + 1, n_samples, taxonomy_id, model_id, ['%.4f' % l for l in test_losses.val()],
                           ['%.4f' % m for m in _metrics_dcd]), logger=logger)
        ...
        for _, v_dcd in category_metrics_dcd.items():
            test_metrics_dcd.update(v_dcd.avg())
        print_log('[TEST] Metrics_DCD = %s' % (['%.4f' % m for m in test_metrics_dcd.avg()]), logger=logger)
