...
def resume_model(base_model, args, logger=None):
    ...
    print_log(f'[RESUME INFO] resume ckpts @ epoch( best_metrics = {str(best_metrics):s})',
              logger=logger)
...
def load_model(base_model, ckpt_path, logger=None):
    ...
    filtered_ckpt = {k: v for k, v in base_ckpt.items() if k in model_state_dict}
    base_model.load_state_dict(filtered_ckpt, strict=False)  # ,strict=False
    ...
    if state_dict.get('metrics') is not None:
        ...
        metrics = 'No Metrics'
    print_log(f'ckpts @ epoch( performance = {str(metrics):s})', logger=logger)
    return