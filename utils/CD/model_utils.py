# Reference: https://github.com/czvvd/SVDFormer_PointSea/blob/main/models/model_utils.py
import torch
...
def fps_subsample_v2(pc, n_points=2048):
    """
    Args
        pcd: (b, 16384, 6)

    returns
        new_pcd: (b, n_points, 6)
    """
    pcd = pc[:, :, :3]
    new_pcd = gather_operation(pc.permute(0, 2, 1).contiguous(), furthest_point_sample(pcd.contiguous(), n_points))
    new_pcd = new_pcd.permute(0, 2, 1).contiguous()
    return new_pcd
...
class CrossViewAttention(nn.Module):
    ...
    def __init__(self, d_model_in1=256, d_model_in2=256, d_model_mid=256, d_model_out=256, nhead=4, dim_feedforward=1024, dropout=0.0):
        super().__init__()
        self.multihead_attn = nn.MultiheadAttention(d_model_mid, nhead, dropout=dropout)
        self.linear11 = nn.Linear(d_model_mid, dim_feedforward)
        self.dropout1 = nn.Dropout(dropout)
        self.linear12 = nn.Linear(dim_feedforward, d_model_out)

        self.norm12 = nn.LayerNorm(d_model_mid)
        self.norm13 = nn.LayerNorm(d_model_mid)
        ...
        self.src1_proj = nn.Conv1d(d_model_in1, d_model_mid, kernel_size=1)
        self.src2_proj = nn.Conv1d(d_model_in2, d_model_mid, kernel_size=1)

        self.out_proj = nn.Linear(d_model_mid, d_model_out)
    def with_pos_embed(self, tensor, pos):
        return tensor if pos is None else tensor + pos

    def forward(self, src1, src2, pos1=None, pos2=None):
        src1 = self.src1_proj(src1)
        src2 = self.src2_proj(src2)
        b, c, _ = src1.shape
        src1 = src1.reshape(b, c, -1).permute(2, 0, 1)
        src2 = src2.reshape(b, c, -1).permute(2, 0, 1)
        src1 = self.norm13(src1)
        src2 = self.norm13(src2)
        q  = self.with_pos_embed(src1, pos1)
        k = self.with_pos_embed(src2, pos2)
        src12 = self.multihead_attn(query=q,
                                     key=k,
                                     value=src2)[0]
        src1 = src1 + self.dropout12(src12)
        src1 = self.norm12(src1)
        src12 = self.linear12(self.dropout1(self.activation1(self.linear11(src1))))
        src1 = self.out_proj(src1) + self.dropout13(src12)
        src1 = src1.permute(1, 2, 0)
...
class PointDecoderBlock(nn.Module):
    def __init__(self, channels, use_emb=False):
        super().__init__()
        self.mlp1 = MLP(in_channel=channels, layer_dims=[channels*2, channels])
        self.norm1 = nn.LayerNorm(channels)
        self.attn = self_attention(d_model=channels, d_model_out=channels, dim_feedforward=channels*2)
        self.norm2 = nn.LayerNorm(channels)
        self.mlp2 = MLP(in_channel=channels, layer_dims=[channels*2, channels])
        self.use_emb = use_emb

    def forward(self, f, emb=None):
        '''
        ...
        :return:
        '''
        f = self.norm1(self.mlp1(f))
        res1 = f
        if self.use_emb:
            f += emb.repeat(1, 1, f.shape[-1])
        f_attn = self.attn(f.transpose(1, 2).contiguous())
        f_out = self.norm2(f_attn.transpose(1, 2).contiguous() + res1)
        res2 = f_out
        f_out = self.mlp2(f_out) + res2
        return f_out

class PointDecoder(nn.Module):
    def __init__(self, channels, channels_add, ratio=1, blk_num=1, use_add_feat=True, use_emb=False):
        super().__init__()
        self.blk_num = blk_num
        self.ratio = ratio
        self.use_add_feat = use_add_feat
        self.blks_main = nn.ModuleList()
        for _ in range(blk_num):
            self.blks_main.append(PointDecoderBlock(channels=channels, use_emb=use_emb))
        if use_add_feat:
            self.blks_add = nn.ModuleList()
            self.map_mlp = MLP(in_channel=channels_add, layer_dims=[channels, channels])
            for _ in range(blk_num):
                self.blks_add.append(PointDecoderBlock(channels=channels, use_emb=False))
        self.out_mlp = MLP(in_channel=channels, layer_dims=[64, 3*ratio])

    def forward(self, f, f_add, emb=None):
        '''
        ...
        for i in range(self.blk_num):
            f = self.blks_main[i](f, emb)
            if self.use_add_feat:
                f_add = self.blks_add[i](f_add, None)
        if self.use_add_feat:
            f_out = f + self.map_mlp(f_add)
        else:
            f_out = f
        delta = self.out_mlp(f_out).reshape(bs, N * self.ratio, -1)
...
class LocalPointEncoder(nn.Module):
    def __init__(self, in_channels=3, out_channels=256):
        super().__init__()
        self.edge_conv1 = EdgeConv(input_channel=in_channels, output_channel=64, k=16)
        self.edge_conv2 = EdgeConv(input_channel=64, output_channel=256, k=8)
        self.conv11 = nn.Conv1d(in_channels=64, out_channels=32, kernel_size=1)
        self.conv12 = nn.Conv1d(in_channels=64, out_channels=32, kernel_size=3, padding=1)
        self.conv13 = nn.Conv1d(in_channels=64, out_channels=32, kernel_size=5, padding=2)
        self.conv21 = nn.Conv1d(in_channels=256, out_channels=32, kernel_size=1)
        self.conv22 = nn.Conv1d(in_channels=256, out_channels=32, kernel_size=3, padding=1)
        self.conv23 = nn.Conv1d(in_channels=256, out_channels=32, kernel_size=5, padding=2)
        self.mlp = MLP_CONV(in_channel=96*2, layer_dims=[out_channels, out_channels])

    def forward(self, x):
        f1 = self.edge_conv1(x)
        f2 = self.edge_conv2(f1)
        f11 = torch.cat([self.conv11(f1), self.conv12(f1), self.conv13(f1)], dim=1)
        f22 = torch.cat([self.conv21(f2), self.conv22(f2), self.conv23(f2)], dim=1)
