    ...
    for key, car_list in Cars_dict.items():
        ...
        for i, this_car in enumerate(car_list):
            ...
            this_car = torch.from_numpy(np.load(
                os.path.join(Data_path, f'frame_{this_frame}_car_{int(this_elements[3])}_{int(this_elements[4]):03d}',
                             'pred.npy'))).unsqueeze(0).cuda()
            next_car = torch.from_numpy(np.load(
                os.path.join(Data_path, f'frame_{next_frame}_car_{int(next_elements[3])}_{int(next_elements[4]):03d}',
                             'pred.npy'))).unsqueeze(0).cuda()
...
def get_MMD():
    ...
    # 将GT数据预先加载到GPU
    gt_data = [ShapeNetCars_dataset[index][-1][1].cuda().unsqueeze(0) for index in range(len(ShapeNetCars_dataset))]

    for item in tqdm(sorted(Samples)):
        pred_data = torch.from_numpy(np.load(os.path.join(Data_path, item, 'pred.npy'))).unsqueeze(0).cuda()
        batch_cd = []

        # 使用torch.no_grad()来加速计算
        with torch.no_grad():
            for gt in gt_data:
...
if __name__ == '__main__':
    ...
    CarsDataloader = torch.utils.data.DataLoader(ShapeNetCars_dataset, batch_size=1, shuffle=False, num_workers=0)
